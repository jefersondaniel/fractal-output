...
class Item(Resource):
    async def apply_transformer(self, selected_includes=None, include_stack=None):
        if not self.data:
            return Result(None)
        ...
        result = await self.transformer.transform(self.data)

        for include in self.transformer.default_includes:
            await self.process_include(result, include, selected_includes, include_stack)
    ...
    async def process_include(self, result, include, selected_includes, include_stack):
        ...
        include_result = await (await method_instance(self.data)).apply_transformer(selected_includes, include_stack + [include])
...
class Collection(Resource):
    async def apply_transformer(self, selected_includes=None, include_stack=None):
        ...
        for value in self.data:
            ...
            result.append((await item.apply_transformer(selected_includes, include_stack)).get_data())
